...
class SolCleaner:
    ...
    def flush_delete(self, with_update=True):
        while True:
            try:
                ...
                sleep(e.seconds + 2)
        self.del_list.clear()
        if with_update: self.update('in operation')
    ...
    def update(self, status, force=False):
        ...
        self.edit("<< clean " + status + " >>\n%d (%s)\n(%dconfrimed, %scheduled)" % (
            self.offset_id,
            datestr,
            self.scanned, self.deleted
        ), force)
